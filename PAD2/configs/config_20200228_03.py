...
def get_config(training=True):
    ...
    conf.train_list = 'anno/4@1_train.txt'

    # path where validation list is, you should change it according to your setting
    conf.val_list = 'anno/4@1_dev_test_res_label.txt'

    # path where test list is, you should change it according to your setting
    # conf.test_list = 'anno/4@1_dev_test_res_label.txt'
    conf.test_list = 'anno/phase2/4@1_test_res.txt'
...
# --------------------Training Config ------------------------
    ...
    conf.train.transform = trans.Compose([  # convert input from PIL.Image to Tensor and normalized
        ...
        trans.ToTensor(),
        # trans.Normalize(mean_, std_)
    ])

# --------------------Inference Config ------------------------
    conf.test = edict()
    # conf.test.save_name = '4@1-dev-single-best-acc-loss'
    conf.test.save_name = '4@1-test-single-best-acc-loss'
    conf.test.epoch = 16
    # conf.test.epoch_start = 150
    # conf.test.epoch_end = 200
    # conf.test.epoch_interval = 8 #we set a range of epoches for testing
    # path for save predict result, pred_result is saved in conf.pred_path/conf.exp, you should change it according to your setting
