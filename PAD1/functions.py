    ...
    def read_images(self, path, selected_folder, use_transform):
        ...
        for i in self.frames:
            image = Image.open(os.path.join(
                path, selected_folder, 'frame{:06d}.jpg'.format(i))).convert('L')
...
class Dataset_CRNN(data.Dataset):
    ...
    def __init__(self, data_path, train_file, num_seq, mode, transform=None):
        "Initialization"
        self.data_path = data_path
    ...
    def read_image_seqs(self, path, selected_folder, filenames, rand_range):
        X = []
        for i in rand_range:
            image = Image.open(os.path.join(
                path, selected_folder, filenames[i % len(filenames)]))  # in case of num_frames <= num_seq
    ...
    def read_images(self, path, selected_folder, use_transform):
        files = sorted(os.listdir(os.path.join(path, selected_folder)))
        num_frames = len(files)  # total frames number of this video track

        if self.mode == 'rand':
            if num_frames > self.num_seq:
                rand_start = np.random.randint(
                    0, num_frames - self.num_seq + 1)
                rand_range = np.arange(rand_start, rand_start + self.num_seq)
            else:
                rand_range = np.arange(self.num_seq)

            X = self.read_image_seqs(path, selected_folder, files, rand_range)
        elif self.mode == 'full':
            X = []
            for i in range(num_frames - self.num_seq + 1):
                rand_range = np.arange(i, i + self.num_seq)
                X_ = self.read_image_seqs(
                    path, selected_folder, files, rand_range)
                X.append(X_)
            X = torch.stack(X, dim=0)
        else:
            raise ValueError
...
if __name__ == "__main__":
    ...
    train_set, valid_set = Dataset_CRNN(data_path, train_file, num_seq, 'rand', transform=train_transform), \
        Dataset_CRNN(data_path, test_file, num_seq,
                     'rand', transform=test_transform)
    ...
    for batch_idx, (x, y) in enumerate(train_loader):
        for b in range(x.shape[0]):
            for t in range(x.shape[1]):
                save_dir = 'dump/track_{:2d}'.format(b)
                if not os.path.exists(save_dir):
                    os.makedirs(save_dir)
                save_name = '%06d.jpg' % (t)
                save_path = os.path.join(save_dir, save_name)
                trans_(x[b, t]).save(save_path)
        break
